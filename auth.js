...
const VetConnectAuth = (() => {
  ...
  // ── LOGIN — server-side via Netlify function ────────────
  ...
  // ── PRESENCE HEARTBEAT — server-side ───────────────────
  function startPresenceHeartbeat(recordId) {
    if (window._presenceInterval) clearInterval(window._presenceInterval);
    function ping() {
      fetch('/.netlify/functions/presence', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  async function createInvite(email, role, invitedBy) {
    try {
      const res = await fetch('/.netlify/functions/invite', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ email: email, role: role, invitedBy: invitedBy })
      });
      const data = await res.json();
      if (!data.success) return { success: false, message: data.message };
      const inviteUrl = window.location.origin + '/signup.html?token=' + data.token;
      return { success: true, inviteUrl: inviteUrl, token: data.token };
    } catch(err) {
  ...
  async function validateInviteToken(token) {
    try {
      const res = await fetch('/.netlify/functions/invite?token=' + encodeURIComponent(token));
  ...
  // ── AIRTABLE QUERY (for platform data — uses localStorage token) ──
  async function query(tableId, filter, opts) {
    opts = opts || {};
    const token = localStorage.getItem('vcg_airtable_token_full');
    if (!token) return { records: [], error: 'No token — set in Settings' };
    var url = 'https://api.airtable.com/v0/appooo5Vcblwu8Ysn/' + tableId +
              '?maxRecords=' + (opts.max || 200) +
  ...
  async function airtableFetch(table, opts) {
    ...
    const url = 'https://api.airtable.com/v0/appooo5Vcblwu8Ysn/' + table + (opts.query || '');
  ...
  return {
    login, logout, getSession, requireAuth, hasRight,
    createInvite, validateInviteToken, startPresenceHeartbeat,
    query, airtableFetch,
    ROLE_PERMISSIONS, ROLE_DEFAULTS, ALL_RIGHTS
  };

})();

// ── AUTH GUARD — skips public pages ────────────────────
(function() {
